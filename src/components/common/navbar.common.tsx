...
const NavbarCommon = () => {
  ...
  const [isMobileMinistriesOpen, setIsMobileMinistriesOpen] =
    useState(false);
  ...
  const canUseTransparentNavbar =
    transparentNavbarRoutes.includes(pathname);
  ...
  return (
    ...
    >
        ...
        <div className="hidden flex-1 justify-center lg:flex">
          <div className="flex items-center gap-6 xl:gap-8">
            ...
            >
              ...
              >
                <div className="grid grid-cols-[0.7fr_1.3fr] gap-10 rounded-xl border border-black/5 bg-white p-8 text-black shadow-[0_20px_60px_rgba(0,0,0,0.12)]">

                  {ministryGroups.map((group) => (
                    <div key={group.title}>

                      <p className="mb-5 text-[10px] font-semibold uppercase tracking-[0.25em] text-green-700">
                        {group.title}
                      </p>
                      ...
                      >
                        {group.items.map((item) => (
                          <Link
                            key={item.label}
                            href={item.href}
                            className="group flex items-center gap-2 text-sm text-black/60 transition duration-200 hover:text-green-700"
                          >
                            <span className="h-px w-0 bg-green-700 transition-all duration-300 group-hover:w-3" />
      ...
      <div
        className={`fixed inset-0 z-[100] bg-[#0a0a0a] transition-transform duration-500 ease-in-out lg:hidden ${
          isMobileMenuOpen
            ? "translate-x-0"
            : "translate-x-full"
        }`}
      >
        ...
        <div className="max-h-[calc(100vh-73px)] overflow-y-auto px-6 pb-12 pt-8">
                    ...
                    <div className="grid gap-3">
                      ...
                      {group.items.map((item) => (
                        <Link
                          ...
                          className="text-base text-white/55 transition hover:text-white"
                        >
                          {item.label}
                        </Link>
                      ))}
