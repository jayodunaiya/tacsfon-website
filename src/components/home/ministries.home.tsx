...
const MinistriesHome = () => {
  const units =
    ministryGroups.find((group) => group.title === "Units")?.items || [];

  const subgroups =
    ministryGroups.find((group) => group.title === "Subgroups")?.items || [];
        ...
        <div className="mb-14 grid gap-8 border-t border-black/10 pt-8 md:grid-cols-2">
          <FadeUp delay={0.15}>
            <p className="max-w-xl text-base leading-7 text-black/55">
              Church is more than attending a service. Our ministries create
              spaces to grow, serve, build relationships and become part of
          ...
          >
            <p className="max-w-sm text-sm leading-6 text-black/40">
              From our core units to specialised subgroups, there&apos;s a
              place for every member to serve and grow.
            </p>
          </FadeUp>
        </div>
              ...
              <StaggerItem key={unit.label}>
                ...
                >
                  ...
                  >
                    ...
                    {/* Hover Green Overlay */}
                    <div className="absolute inset-0 bg-green-800/0 transition-all duration-500 group-hover:bg-green-800/75" />
                    ...
                    {/* Main Content */}
                    <div className="absolute bottom-0 left-0 z-10 flex w-full flex-col justify-end p-6 lg:p-8">

                      {/* Default Content */}
                      <div className="transition-all duration-500 group-hover:-translate-y-24">
                        <p className="mb-3 text-[9px] font-semibold uppercase tracking-[0.25em] text-green-400 group-hover:text-white/60">
                          Ministry Unit
                        </p>

                        <h3 className="max-w-[260px] text-3xl font-medium leading-[1] tracking-[-0.04em] text-white md:text-4xl">
                          {unit.label}
                        </h3>
                      </div>
                      ...
                      {/* Scripture */}
                      {unit.scripture && (
                        <div className="absolute bottom-7 left-6 right-6 translate-y-6 opacity-0 transition-all duration-500 group-hover:translate-y-0 group-hover:opacity-100 lg:bottom-8 lg:left-8 lg:right-8">
                          <div className="border-t border-white/25 pt-5">
                            ...
                            {unit.scriptureRef && (
                              <p className="mt-4 text-[9px] font-semibold uppercase tracking-[0.22em] text-white/50">
                                {unit.scriptureRef}
                              </p>
                            )}
                          </div>
        ...
        <div className="mt-24 lg:mt-32">
          ...
          <Stagger className="border-t border-black/10">
            {subgroups.map((subgroup, index) => (
              <StaggerItem key={subgroup.label}>
                <Link
                  href={subgroup.href}
